 ...
 */
function comment_controller_maybe_show_comments( $user_id, $post_id = false ) {
	$user   = get_userdata( $user_id );
	$return = true;

	if ( $post_id ) {
		$post_type  = get_post_type();
		$post_types = comment_controller()->settings->get_option( 'disabled_post_types', array() );
		$author_id  = get_the_author_meta( 'ID' );
		$author     = get_userdata( $author_id );

		if ( $author->comment_controller_disallow ) {
			$return = false;
		}
	...
	$roles = comment_controller()->settings->get_option( 'disabled_roles', array() );
	...
	if ( ! is_user_logged_in() ) {
		...
		if ( $user->comment_controller_disable ) {
			$return = false;
		}
	}
 ...
 * @return      bool $return True if shown, false otherwise
 */
function comment_controller_maybe_show_profile_option() {
	$return = true;
	$roles  = comment_controller()->settings->get_option( 'disabled_roles', array() );
 ...
 * @return      mixed
 */
function comment_controller_show_comments() {
	global $post, $wp_query;

	$user_id = get_current_user_id();

	if ( ! comment_controller_maybe_show_comments( $user_id, $post->ID ) ) {
		...
		return COMMENT_CONTROLLER_DIR . 'templates/comments.php';
	}
}
add_action( 'comments_template', 'comment_controller_show_comments' );
 ...
 */
function comment_controller_show_comment_form( $open, $post_id ) {
	$user_id = get_current_user_id();

	if ( ! comment_controller_maybe_show_comments( $user_id ) ) {
		$open = false;
	}

	return $open;
}
add_filter( 'comments_open', 'comment_controller_show_comment_form', 999, 2 );
 ...
 */
function comment_controller_get_post_types() {
	$post_types = array();
 ...
 */
function comment_controller_get_roles() {
	global $wp_roles;
